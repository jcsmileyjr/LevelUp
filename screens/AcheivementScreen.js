...
export default class Achievements extends React.Component {
  ...
  loadData = async () => {
    try {
      ...
      if (savedAchievements !== null) {
        this.setState({
          achievements: this.sortAchievementsByDate(savedAchievements),
        }); //load saved achievements to state
      }
    } catch (e) {
  ...
  //Sort list of achievements by date (Strings) before displaying
  sortAchievementsByDate = (savedAchievements) => {
    parsedAchievements = JSON.parse(savedAchievements);

    parsedAchievements.sort((successA, successB) => {
      if (successA.date < successB.date) {
        return -1;
      }
  ...
  render() {
    ...
    return (
      <Container>
            ...
            {this.state.achievements.map((success, index) => {
              return (
                <Row key={index} style={styles.rowStyle}>
                  <Col style={styles.contentStyle} size={79}>
                  ...
                  <Col style={styles.timelineBar} size={1}>
                    <Text></Text>
                  </Col>
                  <Col size={20}>
                    <Text style={styles.dateStyle}>{success.date}</Text>
                  </Col>
                </Row>
              );
            })}
        ...
        <FooterWithButton
          text="Manually Add Completed Milestone"
